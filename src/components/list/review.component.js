  ...
  <div class="vote-container"><sl-button id="downvote-btn" disabled>Downvote</sl-button><div>Downvotes: ${this.review.downvotes}</div></div>`;
    ...
    //Enable voting for non-authors 
      ...
      ${(this.review.upvoters.includes(AuthAPI.currentUser._id) ? 
        html`
       <div class="vote-container"><sl-button id="upvote-btn" disabled @click=${() => {this.voteReview('upvote');}}>Upvote</sl-button><div>Upvotes: ${this.review.upvotes}</div></div> 
        ` : 
        html`
          <div class="vote-container"><sl-button id="upvote-btn" @click=${() => {this.voteReview('upvote');}}>Upvote</sl-button><div>Upvotes: ${this.review.upvotes}</div></div> 
        `)}
      
        ${(this.review.downvoters.includes(AuthAPI.currentUser._id) ? 
        html`
       <div class="vote-container"><sl-button id="downvote-btn" disabled @click=${() => {this.voteReview('downvote');}}>Downvote</sl-button><div>Downvotes: ${this.review.downvotes}</div></div> 
        ` : 
        html`
          <div class="vote-container"><sl-button id="downvote-btn" @click=${() => {this.voteReview('downvote');}}>Downvote</sl-button><div>Downvotes: ${this.review.downvotes}</div></div> 
        `)}`;
    }
  }
